...
static int esmpp_frames_init(AVHWFramesContext *hwfc) {
    AVESMPPFramesContext *avfc = hwfc->hwctx;
    AVESMPPDeviceContext *hwctx = hwfc->device_ctx->hwctx;
    MppBufferType buf_type = MPP_BUFFER_TYPE_DMA_HEAP;
    int i, ret;
    size_t size;
    ...
    buf_type = mpp_buffer_get_type(buf_type, avfc->die_idx);
    ret = mpp_buffer_group_get_internal(&avfc->buf_group, buf_type | hwctx->flags);
    if (ret != MPP_OK) {
