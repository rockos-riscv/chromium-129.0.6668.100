...
static av_cold int mpp_decode_init(AVCodecContext *avctx) {
    ...
    if ((ret = esmpp_create(&r->mctx, MPP_CTX_DEC, coding_type, r->die_idx)) != MPP_OK) {
...
static int mpp_decode_set_buffer_group(AVCodecContext *avctx, enum AVPixelFormat pix_fmt, MppFramePtr mpp_frame) {
    ...
    if (r->buf_group) {
        ...
        MppBufferType type = mpp_buffer_get_type(MPP_BUFFER_TYPE_DMA_HEAP, r->die_idx);
        if ((ret = mpp_buffer_group_get_external(&r->buf_group, type)) != MPP_OK) {
    ...
    for (int i = 0; i < hwfc->initial_pool_size; i++) {
        ...
        buf_info.type = mpp_buffer_get_type(buf_info.type, r->die_idx);
